...
const TweetSourceModal = ({ endModals }) => {
  ...
  const fetchLeads = async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://api.networkx.ai/api/v1/get-leads", {
        method: "POST",
        headers: {
          Accept: "text/csv",
  ...
  return (
    <div className="fixed w-[100vw] h-[100vh] top-0 left-0 flex items-center justify-center">
      <form
        onSubmit={fetchLeads}
        className="max-w-[700px] radient-shadow border-text border-[2px] w-full relative z-[99] bg-[#091324] p-8 py-[40px] rounded-[20px]"
      >
        <p className="font-medium text-center text-lg">Add Post Source</p>
        <div className="flex items-center">
          ...
          <div className="border border-dim rounded-md p-4 py-1 my-3 w-[70%]">
            <input
              ...
              className="text-text placeholder-dim bg-transparent py-2 w-full outline-none text-xs"
            />
          </div>
        </div>
        <div className="flex items-center justify-between">
          <div className="w-[30%] mb-2">
            <h2 className="text-xs flex items-center">Enter a post link:</h2>
            <p className="text-xs text-dim">
              Select a post to scrape leads from
            </p>
          </div>
          <div className="border border-dim rounded-md p-4 py-1 my-3 w-[70%]">
            <input
              type="url"
              onChange={(e) => handleTweetLink(e.target.value)}
        ...
        <div className="flex items-center justify-between">
          <div className="w-[30%] mb-2">
            <h2 className="text-xs flex items-center">Select a lead source:</h2>
            <p className="text-xs text-dim">
              Choose the type of engagement you'd like to scrape
            </p>
          </div>
          <ThemeProvider theme={theme}>
            <FormControl className="w-[70%]">
              ...
              >
                ...
                <MenuItem value={"retweets"} disabled>
                  Likes (Temporary disabled)
                </MenuItem>
                <MenuItem value={"retweets"}>Reposts</MenuItem>
        ...
        <div className="flex items-center">
          ...
          <div className="border w-[70%] border-dim rounded-md p-4 py-1 my-3">
            <input
              type="number"
        ...
        <button
          disabled={tweetLinkError || typeError || !tweetLink || !type}
...
export default TweetSourceModal;

...
const Prompt = ({ setStepIndex }) => {
  const [prompt, setprompt] = useState(
    window.localStorage.getItem("replierPrompt") || ""
  );
  ...
  const saveandContinue = () => {
    window.localStorage.setItem("replierPrompt", prompt);
    setStepIndex(2);
  };
  return (
    <div className="max-w-[700px] w-full mx-auto text-text">
      <div className="bg-[#E1EAFD] w-full mt-2 p-2 min-h-[300px] rounded-md">
        <h1 className="text-black text-center font-bold border-b-[2px] py-2 border-b-[#878C97]">
          Write A Prompt For Your Replies
        </h1>
        <textarea
          onChange={(e) => handlePromptChange(e.target.value)}
          value={prompt}
          placeholder="Enter a detailed prompt to guide the AI model on what to reply to the selected accounts…"
        ...
        <div className="flex justify-between items-center pt-4 h-[10%] border-t-[1px] border-t-[#737373]">
          ...
          <button
            ...
            className="bg-secondary p-2 rounded text-xs"
          >
            Save & Continue
          </button>
        </div>
